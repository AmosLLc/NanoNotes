...
顺序查找一般是基于**无序的数组或者链表**，采用顺序查找方式两者其实差不多。效率很低。需要比较的次数较多。
...
> 命题：在 N 个键的有序数组中进行二分查找最多需要 (logN + 1) 次比较（无论是否成功）。
...
findVal > arr[mid] , 说明要查找的数在 mid 的右边, 因此需要**递归**的向**右**查找

findVal < arr[mid], 说明要查找的数在 mid 的左边, 因此需要**递归**的向**左**查找
