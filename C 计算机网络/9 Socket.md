...
![1563596580857](assets/1563596580857.png)
...
由于 CPU 要处理更多的系统调用，因此这种模型的 CPU 利用率比较**低**。

![1563596593363](assets/1563596593363.png)
...
![1563596605006](assets/1563596605006.png)
...
![1563596629511](assets/1563596629511.png)
...
![1563596642259](assets/1563596642259.png)
...
- select 的描述符类型使用**数组实现**，FD_SETSIZE 大小默认为 **1024**，因此默认只能监听 1024 个描述符。如果要监听更多描述符的话，需要修改 FD_SETSIZE 之后**重新编译**；而 poll 的描述符类型使用**链表实现**，没有描述符数量的限制；
...
- select 和 poll 每次调用都需要将**全部描述符**从应用进程缓冲区复制到内核缓冲区。
