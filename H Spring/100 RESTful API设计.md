...
1. 每一个 **URI 代表一种资源**；
2. 客户端和服务器之间，传递这种资源的某种**表现形式**比如 json，xml，image,txt 等等；
...
2. 不用大写字母，建议**用中杠 - 不用下杠 \_**。比如邀请码写成 `invitation-code` 而不是 ~~invitation_code~~。
...
如果我们在查询的时候需要**添加特定条件**的话，建议使用 **url 参数**的形式。比如我们要查询 state 状态为 active 并且 name 为 guidegege 的班级：
