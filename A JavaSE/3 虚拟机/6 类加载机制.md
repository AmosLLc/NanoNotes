...
**②通过==数组定义==引用类，不会触发此类的初始化；该过程会对数组类进行初始化，数组类是一个由虚拟机自动生成的、直接继承自 Object 的子类，其中包含了数组的属性和方法。**
...
类加载器在类层次划分、OSGi、热部署和代码加密等领域有重要作用。
