...
- 字符串**修改较为频繁**时使用，StringBuilder 与 StringBuffer 实现几乎相同，只是StringBuilder 线程不安全（不能同步访问）， StringBuffer 线程安全。由于 StringBuilder 相较于 StringBuffer 有速度优势，**所以多数情况下建议使用 StringBuilder 类**。然而在应用程序要求线程安全的情况下，则必须使用 StringBuffer 类。
- 修改时不产生新的未使用变量。
- 继承体系如下图所示
...
- 使用 **Arrays.sort()** 方法对**数值型**数组排序，使用的是**快速排序**算法。
...
在覆写 equals() 方法时应当**总是覆写** hashCode() 方法，保证等价的两个对象散列值也**相等**，一般两个需要**同时**覆写。
