...
我们将通过静态代理对 UserServiceImpl 进行功能**增强**，在调用 `select` 和 `update` 之前记录一些**日志**。写一个代理类 **UserServiceProxy**，代理类需要实现 **UserService**.
...
通过静态代理，我们达到了**功能增强**的目的，而且没有侵入原代码，这是静态代理的一个优点。
...
编写一个 **LogInterceptor** ，继承了 **MethodInterceptor**，用于方法的拦截回调。在 intercept 方法中对业务代码进行增强。
...
还可以进一步实现**多个 MethodInterceptor** 进行过滤筛选
